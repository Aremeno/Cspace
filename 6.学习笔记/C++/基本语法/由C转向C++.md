...
命名空间主要是为了**解决命名冲突**的问题。在大型项目中，不同的库可能会定义相同名称的变量、函数或类，使用命名空间可以将它们区分开来。
头文件中（就是库，比如`<iostream>`）通常会使用命名空间来组织代码。例如，C++标准库中的所有内容都位于`std`命名空间中。
...
**学到这里足以见得,相比于C语言还需要手动实现这些数据结构,C++真的是太方便了!**